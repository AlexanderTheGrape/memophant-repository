...
def openMainMenu(frame):
	...
	#GridLayout(5 rows, 5 cols,  20 hgap, 20 vgap)

	screenWidth = root.winfo_screenwidth()
	screenHeight = root.winfo_screenheight()

	print('width/3:', int(screenHeight/3), 'note: labels do with and height according to font size')

	emptyText = Label(frame, bg='red', pady=20, width = 10, height = 1)
	emptyText.grid(row=0, columnspan=3)

	photo = PhotoImage(file="Slovakia.jpg")

	label = Label(image=photo)
	label.image = photo
	label.grid(row=0, column=0)

	#emptyText = Label(frame, bg='blue', fg="black", text= "", font="Calibri 12 bold", pady=20, width = 200)
	#emptyText.grid(row=0, column=0, rowspan=3)

	#memophantLogo = Label(frame, fg="black", text= "Main Menu", font="Calibri 12 bold", padx=200, pady=20)
	#memophantLogo.grid()

	#mainMenuText = Label(frame, fg="black", compound=CENTER, text= "Main Menu", font="Calibri 12 bold", pady=20)
	#mainMenuText.grid(row=0, column=0, columnspan=2)

	#learnButton = Button(frame, padx=32, pady=16, bd=4, text="Learn", fg="black", font='Calibri 12 bold', command= lambda: openLearningListOptions(frame))
	#learnButton.grid(row=2, column=2)

	#newLearningListButton = Button(frame, padx=32, pady=16, bd=4, text="New Learning List", fg="black", font='Calibri 12 bold', command= lambda: newLearningList(frame))
	#newLearningListButton.grid(row=3, column=3)
...
def openLearningListOptions(frame):
	...
	for i in fileNameList:
		listButton = Button(frame, padx=32, pady=16, bd=4, text="%s" %(i), fg="black", font='Calibri 12 bold', command= lambda i=i: openLearningListContent(frame, i))
		listButton.pack()
...
def selfCheckAnswer(frame, questionCounter, QsAndAs, userAnswer, sessionAnswerInfo):
	...
	noButton = Button(frame, padx=32, pady=16, bd=4, text="No", fg="black", font='Calibri 12 bold', command= lambda: nextQuestion(frame, questionCounter, QsAndAs, sessionAnswerInfo))
	noButton.bind('<Return>', lambda event: nextQuestion(frame, questionCounter, QsAndAs, sessionAnswerInfo))
	noButton.pack()
	noButton.focus()

	yesButton = Button(frame, padx=32, pady=16, bd=4, text="Yes", fg="black", font='Calibri 12 bold', command= lambda: nextQuestion(frame, questionCounter, QsAndAs, sessionAnswerInfo))
	yesButton.bind('<Return>', lambda event: nextQuestion(frame, questionCounter, QsAndAs, sessionAnswerInfo))
	yesButton.pack()
...
def newLearningList(frame):
	...
	createListButton =  Button(frame, padx=32, pady=16, bd=4, text="Create List", fg="black", font='Calibri 12 bold', command= lambda: instructionsPage(frame))
	createListButton.pack()

	preMadeListButton = Button(frame, padx=32, pady=16, bd=4, text="Premade List", fg="black", font='Calibri 12 bold', command= lambda: testingScreen(frame))
