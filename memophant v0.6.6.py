...
def openLearningListOptions(frame):
	...
	f = []
	for (dirpath, dirnames, filenames) in walk('C:\Program Files (x86)\Memophant\learningSets'):
		f.extend(filenames)
		break
...
#f.write(json.dumps(new_entry) + '\n')
	...
	paragraphCheck = 0
	debugCounter = 0
	with open('C:\Program Files (x86)\Memophant\learningSets\\' + j) as f:
		for line in f:
			debugCounter += 1
			try:
...
def listCompleteScreen(frame, questionCounter, QsAndAs, sessionAnswerInfo):
	...
	with open(('C:\Program Files (x86)\Memophant\learningSets\\' + fileName), 'r+') as f:
		for i in QsAndAs:
...
def writeToFile(enteredText, frame):
	...
	completeName = os.path.join('C:\Program Files (x86)\Memophant\learningSets', enteredText + ".txt")
	global newFile
