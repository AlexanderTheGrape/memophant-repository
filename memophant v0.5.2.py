...
def openMainMenu(frame):
	...
	#logo = PhotoImage(file="memophant logo2 gif.gif")
	#logo2 = ImageTk.PhotoImage(Image.open("memophant logo gif.gif"))

	#logoLabel = Label(frame, )

#	img = PhotoImage(Image.open(logo))
	#logo.pack()
...
#			popCounter += 1
#
#	print('popCounter is:', popCounter)
#
...
#	deletingCounter = 0
#	lastFiveValues = (answerHistory[questionCounter])[numberTestingCounter:]
#	print('the last five values used are:', lastFiveValues)
#	while lastFiveAnswersHistory != '***No value***':
#		try:
#			for i in (answerHistory[questionCounter])[numberTestingCounter:]:
#				if i != 'correct' or i != 'incorrect':
#					del lastFiveValues[deletingCounter]
#					deletingCounter += 1
#
...
#	except:
#		try:
#			percentageOfCorrectAnswers = ((100/(timesAnsweredCorrectly + timesAnsweredIncorrectly))*(timesAnsweredCorrectly))
#			percentageText = Label(frame, text=("Overall from all attempts in answering this question, it has been answered correctly %s" % (percentageOfCorrectAnswers) + "% of the time".format()), font="Calibri 12 bold", pady=20, padx=20)
#			percentageText.pack()
#		except:
